...
2. **domain_guard** — an LLM specialist classifies the question as `in_scope`, `clarify`, or `abstain` against `context/semantics.yaml`, and also classifies which `sources` (`sql` and/or `rag`) the question needs.
3. **document_router** — reads that `sources` classification (no keyword scan) and translates it into `active_sources`, dropping `rag` if no complaint retriever is configured. When RAG is needed it routes to `complaint_retrieval` first; otherwise it goes straight into the SQL branch (`planner`).
4. **complaint_retrieval** (only when `rag` is active) — embeds the question, retrieves via `ComplaintRetrievalAdapter` ([app/agents/complaints/retrieval.py](app/agents/complaints/retrieval.py)), which filters to `visibility: public` payloads, allowlists a fixed safe-field set, and redacts text before it ever reaches a response. Never touches SQL/MotherDuck. It runs as a sequential prefix step, not a true parallel branch: for a hybrid request it then chains into the SQL branch (`planner`); for a RAG-only request it goes straight to `merge_results`.
...
9. **merge_results** — sits on every post-router path (whether SQL ran, RAG ran, or both) and combines whichever branch(es) actually ran into the final `answer`/`route` before suggestions runs; when both ran, the SQL section and a `"Consumer complaint narratives found: ..."`-prefixed RAG section are concatenated.
10. **suggestions** — always runs last (every path converges here), producing contextual follow-up questions.

Routing state (`route`, `reason_code`, `repairs`, `executions`, `specialist_path`, `sources`, `active_sources`, `sql_route`, `sql_answer`, `rag_route`, `rag_message`) flows through `AgentState`; `specialist_path` accumulates node names via `Annotated[list[str], add]` for audit/eval purposes. Every node is wrapped in `_traced()` for telemetry spans (Langfuse, or a no-op).
